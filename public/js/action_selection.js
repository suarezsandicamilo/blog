document.getElementById('delete_posts_btn').addEventListener('click', () => {
  // Redirigir a la página de usuario
  window.location.href = '/admin'; // Reemplaza '/user' con la ruta de la página del usuario
});

document.getElementById('delete_comments_btn').addEventListener('click', () => {
  // Redirigir a la página de administrador
  window.location.href = '/admin'; // Reemplaza '/admin' con la ruta de la página de administrador
});

document.getElementById('delete_authors_btn').addEventListener('click', () => {
  // Redirigir a la página de administrador
  window.location.href = '/admin'; // Reemplaza '/admin' con la ruta de la página de administrador
});

document.getElementById('delete_categories_btn').addEventListener('click', () => {
  // Redirigir a la página de administrador
  window.location.href = '/admin'; // Reemplaza '/admin' con la ruta de la página de administrador
});
